...
public class Block<T extends Tx>
{
    ...
    public Block<T> Clone()
    {
        ...
        // There are more efficent ways but this is the most readable
    ...
    /*
     * This method was adapted from the https://github.com/bitcoinj/bitcoinj
     * project Copyright 2011 Google Inc. Copyright 2014 Andreas Schildbach
     * 
     ...
     * 
     * The tree is represented as a list: t1,t2,t3,t4,A,B,root where each entry
     * is a hash.
     * 
     ...
     * included into a block without having to provide the full block contents.
     * Instead, you can provide only a Merkle branch. For example to prove tx2
     * was in a block you can just provide tx2, the hash(tx1) and A. Now the
     * other party has everything they need to derive the root, which can be
     * checked against the block header. These proofs aren't used right now but
     * will be helpful later when we want to download partial block contents.
     * 
     * Note that if the number of transactions is not even the last tx is
     * repeated to make it so (see tx3 above). A tree with 5 transactions would
     * look like this:
     * 
    ...
    public List<String> merkleTree()
    {
        ...
        // Iterate through each level, stopping when we reach the root
        // (levelSize == 1).
        for (int levelSize = transactions
                .size(); levelSize > 1; levelSize = (levelSize + 1) / 2)
        {
    ...
    public void computeHash()
    {
        ...
        setHash(SHA256.generateHash(timeStamp + index + merkleRoot
                + serializedData + nonce + previousHash));
    }
